phi = inline('z^2 + (0.36)');   % Define the function whose fixed points we seek.
fixpt1 = 0.5+1i*(sqrt(11)/10);     % These are the fixed points.
fixpt2 = 0.5-1i*(sqrt(11)/10);

colormap([1 0 0; 1 1 1]);     % Points numbered 1 (inside) will be colored red;
                              %   those numbered 2 (outside) will be colored white.
M = 2*ones(400,400);          % Initialize array of point colors to 2 (white).


for j=1:400, % Try initial values with imaginary parts between
  y = -2 + (j-1)*.01;        %   -0.7 and 0.7
  for i=1:400,                % and with real parts between
    x = -2 + (i-1)*.01;     %   -1.8 and 1.8.
...
image([-10 10],[-10 10],M),  % This plots the results.
